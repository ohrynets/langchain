...
class JobProcessor:
    ...
    def job_finder_1(self, job_description):
        with get_openai_callback() as cb:
            ...
            job_parcer_output = parser.parse(output)
...
if __name__ == "__main__":
    ...
    template = """/
    Please extract the technical and soft skills from the following job description and store it tech_skills, soft_skills, technologies.
    ...
    list_jobs = []
    for skill in job_desc_store.j:
        skills = procs.job_finder_1(skill)
        list_jobs.append(skills)
    print(list_jobs)
